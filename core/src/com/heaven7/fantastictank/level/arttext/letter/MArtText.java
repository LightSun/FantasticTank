...
public class MArtText extends ArtText {
	...
	public List<Vector2> layout() {
		...
		vecs.add(VECTOR_POOL.obtain().set(p.x + UNIT_WIDTH*6, p.y +UNIT_HEIGHT));
		vecs.add(VECTOR_POOL.obtain().set(p.x + UNIT_WIDTH*6, p.y+UNIT_HEIGHT *2));
		vecs.add(VECTOR_POOL.obtain().set(p.x + UNIT_WIDTH*6, p.y+UNIT_HEIGHT *3));
		//"/\"
		...
		vecs.add(VECTOR_POOL.obtain().set(p.x +UNIT_WIDTH*5, p.y+UNIT_HEIGHT*2 ));
		return vecs;
	}
