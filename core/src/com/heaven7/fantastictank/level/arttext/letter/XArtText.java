...
public class XArtText extends ArtText {
	...
	public List<Vector2> layout() {
		...
		vecs.add(VECTOR_POOL.obtain().set(p.x+UNIT_WIDTH*4, p.y +UNIT_HEIGHT*4));
		//"\"
		vecs.add(VECTOR_POOL.obtain().set(p.x, p.y +UNIT_HEIGHT *4));
		vecs.add(VECTOR_POOL.obtain().set(p.x +UNIT_WIDTH, p.y +UNIT_HEIGHT *3));
		vecs.add(VECTOR_POOL.obtain().set(p.x +UNIT_WIDTH*3, p.y +UNIT_HEIGHT *1));
